        ...
        <slideout width="60%" title="编辑" v-model="show" @on-after-hide="back">
            <template slot="body">
                <ct-form v-if="!error" v-loading="loading" :model="formData" ref="forms" :rules="rules">
                    ...
                    {{#if_is @type 'select'}}<form-item v-model="formData.{{@field}}" type="{{@type}}" label="{{@label}}" :list="typeList" defaultSelect></form-item>{{else if_is @type 'autoComplete'}}<form-item v-model="formData.{{@field}}" type="{{@type}}" label="{{@label}}" :list="typeList" :matchKeys="['key','val']" :keys="['key','val']" :showKeys="['key','val']"></form-item>{{else if_is @type 'custom'}}<form-item label="{{@label}}"></form-item>{{else if_is @type 'checkbox'}}<form-item v-model="formData.{{@field}}" type="{{@type}}" label="{{@label}}" :list="typeList"></form-item>{{else if_is @type 'radio'}}<form-item v-model="formData.{{@field}}" type="{{@type}}" label="{{@label}}" :list="typeList"></form-item>{{else}}<form-item v-model="formData.{{@field}}" type="{{@type}}" label="{{@label}}"></form-item>{{/if_is}}
                    {{/each_handle}}
